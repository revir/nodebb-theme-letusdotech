...
$(document).ready(function() {
    ...
    function setupCategoryTree() {
        $(window).on('action:category.loaded', function () {
            var categoryTree = ajaxify.data.categoryTree;
            var listEl = $('.ul-category-tree');
            if (!listEl.length || !categoryTree || !categoryTree.length) {
                return;
            }
            categoryTree.sort(function(a, b) {
                return a.order - b.order;
            }).forEach(function (category, i) {
                var preOrder = ajaxify.data.hasPreOrder ? ''+(i+1) : '';
                recursive(category, listEl, '', preOrder);
            });
        });

        function recursive(category, listEl, level, preOrder) {
            if (category.link) {
                return;
            }
            var href = config.relative_path + '/category/' + category.slug;
            var liEl = $('<li data-cid="' + category.cid + '"></li>');
            var preOrderSpan = '<span class="tree-pre-order">' + preOrder + '&nbsp;' + '</span>';
            liEl.append('<a href="'+href+'" class="topic-category-name '
                + (category.cid === ajaxify.data.cid ? "active " : '')
                + (category.noPrivilege ? 'disabled' : '') + '">'+ level + preOrderSpan + category.name+'</a>');
            listEl.append(liEl);
            ...
            category.children.sort(function(a, b) {
                ...
                recursive(child, listEl, '&nbsp;&nbsp;&nbsp;&nbsp;' + level, preOrder ? preOrder+'.'+(i+1) : '');
            });
        }
    }
});
